...
- For a clean surface-lightmap refresh, first remove the existing source `lightmap` objects and `surface-lightmap*` runtime files, then run the GPU bake path so stale embedded payloads cannot be reused.
