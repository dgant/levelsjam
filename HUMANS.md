...
- Re-run `npx playwright test tests/smoke.spec.cjs --reporter=line` after touching volumetric fog, probe-occlusion wiring, or lens flares. The smoke test now checks the fog debug state and the maze-connectivity path used by runtime probe blending.
...
- Refresh only the main story path lighting with `LEVELSJAM_SYNC_STORY_LIGHTING=1 node scripts/sync-maze-runtime-data.cjs`, then run `npm run export:maze-probes -- <comma-separated-story-level-ids>` for the authored rooms and story mazes. Story lighting sync reuses matching GPU-baked authored payloads and defaults to 64px wall lightmap tiles with 32 indirect rays; override with `LEVELSJAM_STORY_LIGHTING_WALL_TILE_SIZE` and `LEVELSJAM_STORY_LIGHTING_INDIRECT_RAYS` if needed. Add `LEVELSJAM_SYNC_STORY_LIGHTING_CPU=1` only for diagnostics when the GPU lightmap worker is unavailable or timing out.
...
- The perf test also runs repeated live `W` movement down Chamber 1 and writes timestamped `logs/chamber-1-run-profile-*.md` / `.json` artifacts when diagnosing stutter. Inspect the markdown first; the JSON contains every captured frame sample.
...
- Each `npm run ensure:mazes` or standalone `npm run export:maze-probes` run writes a timestamped `logs/bake-timing-*.json` report with per-level, per-step, quality-parameter, and work-count timing data. Inspect that file before rerunning a bake that timed out.
- Reflection-probe export defaults to using a complete existing probe manifest as the two-pass bootstrap source when one is available; set `LEVELSJAM_REFLECTION_BOOTSTRAP_EXISTING=0` to force a fresh first pass, or `LEVELSJAM_REFLECTION_TWO_PASS=0` for a diagnostic single-pass export.
- Bake subprocesses are bounded by timeouts and should clean up their headless Chrome/server process trees on failure; do not start a full bake with those safeguards disabled.
