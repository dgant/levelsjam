...
test('level menu categories match the requested gameplay grouping', () => {
  ...
  assert.match(appSource, />\s*Graphics\s*<\/button>[\s\S]*>\s*Audio\s*<\/button>[\s\S]*>\s*Gameplay\s*<\/button>[\s\S]*>\s*Skip\s*<\/button>[\s\S]*>\s*Credits\s*<\/button>/)
  ...
  assert.match(appSource, /Watch a speedrun of the whole game/)
  assert.match(appSource, /onBloomSettingChange\(\{ enabled: event\.target\.checked \}\)/)
  assert.match(appSource, /onDepthOfFieldSettingChange\(\{ enabled: event\.target\.checked \}\)/)
  assert.match(appSource, /onLensFlareSettingChange\(\{ enabled: event\.target\.checked \}\)/)
  assert.match(appSource, /onChromaticAberrationSettingChange\(\{ enabled: event\.target\.checked \}\)/)
  assert.match(stylesSource, /\.credits-modal \{[\s\S]*?background: #000;/)
  assert.match(stylesSource, /\.credits-list \{[\s\S]*?column-count: 3;/)
  assert.match(stylesSource, /\.endgame-fade-overlay/)
...
test('save resume, level menu reset, and debug overlay visibility are wired', () => {
  ...
  assert.match(appSource, /ENDGAME_CAMERA_ROTATION_DURATION_MS = 3000/)
  assert.match(appSource, /ENDGAME_FADE_DURATION_MS = 3000/)
  assert.match(specSource, /rotates the camera `180` degrees toward the entrance over `3s`/)
