...
- The surface-lightmap bake is generated by a GPU-backed WebGL lightmapper rather than by per-texel CPU ray tests.
- The source tree contains no CPU surface-lightmap baking fallback that can regenerate wall or floor lightmap texels without the GPU worker.
- Persisted surface-lightmap payloads are regenerated from the GPU worker after old payloads are removed, so committed maze data cannot carry forward stale CPU-baked lightmap bytes.
- The GPU surface-lightmap bake renders atlas rectangles in lightmap UV space and evaluates finite torch emitters, wall/sconce occlusion, skylight visibility, and diffuse global illumination in shader code.
- The GPU surface-lightmap bake is based on the texture-space rasterization and accumulation approach used by Evan Wallace's WebGL lightmap generation article, the official three.js `ProgressiveLightMap` helper, and GPU path-tracing practice for sampled area lights and fixed bounce counts.
- Each torch light is baked as a true finite spherical emitter rather than as a point light with only a distance clamp.
- Each spherical torch emitter uses the same `0.125m` source radius as the visible wall sconce radius.
- Each baked maze lightmap stores the baked static direct torch lighting, direct skylight, and two diffuse light bounces for the maze floor and wall faces.
...
- The baked sconce-body shadow is produced by geometric occlusion from the modeled lower-hemisphere cap and should read as a soft shadow near the fixture rather than an artificial full-height painted contact stripe.
...
- Offline volumetric-lightmap coefficients are generated from the baked maze lighting model directly, including occluded torch contribution and occluded skylight, rather than inferred only from reflection beauty captures.
