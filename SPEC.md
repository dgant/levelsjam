...
- The cheat menu lists every authored story room and every storyline maze used by Chamber 1 and Chamber 2 so each can be directly loaded for emergency testing.
...
- A seamless level transition may commit when the destination level's surface lightmap is ready and the current active level's lighting resources are stable; inactive destination reflection probes do not block the transition because they become active-resident after the handoff.
...
- Custom turn-action events generated by touch controls, automated walkthroughs, or UI helpers are timestamped when received so they are accepted after gameplay input is enabled.
...
- Authored progression levels use explicit non-door `levelConnections` for traversable seams where `LEVELS.md` does not draw a door.
- Every authored `levelExit` renders as a physical door; non-door traversal must not be represented as a hidden or invisible door.
- Authored hallway torch markers in `LEVELS.md` appear as runtime wall torches on the indicated wall faces, including the lower interior torch in `Hallway 1-1`.
...
- `Hallway 1-2` has a single upper-row cell containing the minotaur, a lower single ingress cell, with the exit door on the upper cell's north side and torch walls on both the west and east sides of that upper cell.
- `Hallway 1-3` follows the `LEVELS.md` gate tutorial diagram with a single upper minotaur cell, a lower single ingress cell, a four-cell middle row, a four-cell gate row, and one passable gated edge between the second and third cells of the gate row.
- `Hallway 1-3` has a recorded solution that reaches the `Hallway 1-4` exit under the normal monster and gate rules.
- `Hallway 1-4` follows the revised sword tutorial bend from `LEVELS.md`, starts the player in the lower-right ingress cell, provides a sword in the entry bend before the minotaur engagement, and contains no trophy pickup.
...
- The gameplay-menu walkthrough option progresses by sending normal turn actions after gameplay input is ready, accelerates movement and altar cutscene timers through the debug animation multiplier, and automated coverage verifies that it reaches the final credits screen from `Entrance` without teleporting between story levels.
