...
- The GPU surface-lightmap bake renders atlas rectangles in lightmap UV space and evaluates torch direct lighting, wall/sconce occlusion, and skylight visibility in shader code.
- The GPU surface-lightmap bake is based on the texture-space rasterization and accumulation approach used by Evan Wallace's WebGL lightmap generation article and the official three.js `ProgressiveLightMap` helper.
