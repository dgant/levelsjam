...
[] Replace Werewolf model with the one in leowolf.zip. It should scale proportionally to 1.8m wide and placed so the bottoms of its feet are right on the floor
[] Add credit: "leowolf" (https://skfb.ly/pqPJx) by MUSHIDO_SKARSGARD is licensed under Creative Commons Attribution (http://creativecommons.org/licenses/by/4.0/).
[] The branch "gpu-lightmapper" implements GPU path traced lightmapping to replace the current CPU lightmapping. Integrate it into main. Remove the debug dropdown for choosing different lightmap sources and just use the more recent 
...
[] Continue breaking down the performance.MD hierarchy and then try to cut down on each of the expensive bits. Really understand why N8AO is doing so much on CPU. If it's capturing real work, that real work is likely stupid because N8AO shouldn't need to do much at all on CPU
[] Eyes: Use editable text field instead of slider
[] Flip Werewolf model 180 degrees around the vertical axis (it's facing backwards). Scale it up proportionally 2x
[] From the perspective of whichever wall-sidedness the spider in maze-005 is, the default spider eye settings (XYZ) should be: Left (-.18, .35, -.29), Right (-.27, .26, -.29). The spider model is not symmetric, so the other-sided spider needs its own eye settings based on transforming these values correctly
[] Move that spider's origin (and by extension its eyes, after applying the above defaults), by (.1, 0, .5)
[] Lens flare new default settings:
  [] Enabled
  [] Lens Flares 0.002
  [] Flare Opacity 0.01
  [] Flare size 0.0015
  [] Star Points 3
  [] Secondary Ghosts enabled
[] Lens flare intensity needs a manual text input. I feel like there's some crazy scaling being applied to it right now because the monster eyes produce massive flares at Lens Flares 0.002 and Flare Opacity 0.01
[] The vignette noise formula seems like it's clamping the outputs and causing half the values to be zero. I'm gonna place a bet that the original noise returns [-1, 1] and it's getting clamped to [0, 1]. We want the noise intensity to be additive to the base intensity but we do not want the noise clamped, we want it mapped smoothly to the [0, 1] scale
[] Divide eye radius by 4
[] Default tonemapper to Neutral
[] When I walk into a level the volumetric fog suddenly changes, as if it wasn't sampling the light in that maze until I walked into it. That shouldn't happen.
[] Like gates, doors are closed by default and should appear that way
[] I've noticed it's possible for the camera movement to suddenly jerk and backtrack to a previous position and then back again. It's a very distracting flicker that I assume is a bug in the player/camera position tweening. I have a suspicion that perhaps it's due to an implementation that foolishly lets the rules engine follow commands immediately and tries to catch up with the camera, rather than only stepping the rules engine forward when the appropriate animations have completed. If that suspicion is correct, resolve it by changing to the sounder lockstep operation where animation does not lag the game state; if not, just figure out what conditions would make camera backtracking possible and fix it.
[] Before each move, spiders should turn to face the direction they're going to move in, then after the move, they should turn to face the next direction they will move. Animate the turns if they are nonzero.
[] When I walk through a gate, up to another gate, which then opens, the second gate appears to snap open instead of smoothly animating
[] Monsters should never go to sleep if they have line of sight of the player. Monsters have line of sight through gates. When a minotaur is taking its turn, and it has line of sight with the player through a gate that is blocking it, it should "attempt" the move and fail (animate a failed move the same way we animate it for the player trying to walk through a wall).
