...
[x] Replace Werewolf model with the one in leowolf.zip. It should scale proportionally to 1.8m wide and placed so the bottoms of its feet are right on the floor
[x] Add credit: "leowolf" (https://skfb.ly/pqPJx) by MUSHIDO_SKARSGARD is licensed under Creative Commons Attribution (http://creativecommons.org/licenses/by/4.0/).
[x] The branch "gpu-lightmapper" implements GPU path traced lightmapping to replace the current CPU lightmapping. Integrate it into main. Remove the debug dropdown for choosing different lightmap sources and just use the more recent 
[x] New game element: Doors. They follow the same rules as gates. Each maze's entrance should be a door. The door is represented by two cubes, 1m long, 2m tall, 50% the width of a wall. When closed, the two doors are positioned together along the edge to form a 2mx2m closed door. When they open, they each slide out .75m along the edge, forming a 1.5m wide opening. The opening and closing animations should be 250ms each. Again, these work exactly like gates except using different geometry and sliding behavior. Use textures/metal_rust-1K with the same texel density as walls.
[x] Add appropriately formatted credit for https://www.sharetextures.com/textures/metal/metal-rust
[x] Give monsters two glowing eyes. Each eye is a 3cm red sphere at (4, 0, 0) (where (1, 1, 1) is white; we want them to proc bloom). The eyeballs should be lens flare sources (this is why we wanted to support multiple lens flares). Try to guess the correct positions, which will be in front of the eyeballs of the models. Add a debug menu for eyeballs that lets me manually tweak the relative position of each monster type's eyeballs from 0-2m in each direction so I can give you manually corrected offsets. Render the eyeballs when the monster is awake, and don't when they're asleep. Naturally, the relative position of the eyeballs should rotate along with the monster
[x] On mobile, display touch controls along the bottom of the screen. A left-arrow, an up arrow, and a right arrow for turn left/move forward/turn right (it's okay that there's none to move backward; it's not important). The actual clickable area of the buttons should cover the entire screen. Also add a menu button to the top left which opens the menu (what's currently the level jump menu, but that will change)
[x] Make animation speeds dynamic, relative to the number of queued commands. If animating the player's only command, animate at 1x. If the player has one backlogged command, animate at 1.5x. Two backlogged commands, 2.0x, Three, 2.5x, and so forth. Apply this to all animations including monsters, gates, doors, etc.
[x] We're using analytics in the Daily Dub website in /e/p/dailywin. We're going to use the same analytics host to track events for Minotaur. Track page views, time spent in-game (count this once per minute, if the player made any moves within the last minute), player visits to each level by name, and mazes completed (by walking out of the maze's level while carrying a trophy)
[x] See what you can do about reducing the costs of volumetric fog. I would guess the biggest opportunity is reducing the cost of each lighting sample.
[x] Continue breaking down the performance.MD hierarchy and then try to cut down on each of the expensive bits. Really understand why N8AO is doing so much on CPU. If it's capturing real work, that real work is likely stupid because N8AO shouldn't need to do much at all on CPU
[x] Eyes: Use editable text field instead of slider
[x] Flip Werewolf model 180 degrees around the vertical axis (it's facing backwards). Scale it up proportionally 2x
[x] From the perspective of whichever wall-sidedness the spider in maze-005 is, the default spider eye settings (XYZ) should be: Left (-.18, .35, -.29), Right (-.27, .26, -.29). The spider model is not symmetric, so the other-sided spider needs its own eye settings based on transforming these values correctly
[x] Move that spider's origin (and by extension its eyes, after applying the above defaults), by (.1, 0, .5)
[x] Lens flare new default settings:
  [x] Enabled
  [x] Lens Flares 0.002
  [x] Flare Opacity 0.01
  [x] Flare size 0.0015
  [x] Star Points 3
  [x] Secondary Ghosts enabled
[x] Lens flare intensity needs a manual text input. I feel like there's some crazy scaling being applied to it right now because the monster eyes produce massive flares at Lens Flares 0.002 and Flare Opacity 0.01
[x] The vignette noise formula seems like it's clamping the outputs and causing half the values to be zero. I'm gonna place a bet that the original noise returns [-1, 1] and it's getting clamped to [0, 1]. We want the noise intensity to be additive to the base intensity but we do not want the noise clamped, we want it mapped smoothly to the [0, 1] scale
[x] Divide monster eyeball radius by 4
[x] Default tonemapper to Neutral
[x] When I walk into a level the volumetric fog suddenly changes. I can see that the fog is only lit in a volume bove the current level. For example, I walk into maze-002, and instantly every point over Chamber 1 has dark unlit fog. This is wrong and suggests that, once again, the volumetric lightmap sampling has some considerations for what level we're in, when it should not know or care what level we're in, just what the volumetric lightmap for any particular world cell is.
[x] Like gates, doors are closed by default and should appear that way
[x] I've noticed it's possible for the camera movement to suddenly jerk and backtrack to a previous position and then back again. It's a very distracting flicker that I assume is a bug in the player/camera position tweening. I have a suspicion that perhaps it's due to an implementation that foolishly lets the rules engine follow commands immediately and tries to catch up with the camera, rather than only stepping the rules engine forward when the appropriate animations have completed. If that suspicion is correct, resolve it by changing to the sounder lockstep operation where animation does not lag the game state; if not, just figure out what conditions would make camera backtracking possible and fix it.
[x] Before each move, spiders should turn to face the direction they're going to move in, then after the move, they should turn to face the next direction they will move. Animate the turns if they are nonzero.
[x] When I walk through a gate, up to another gate, which then opens, the second gate appears to snap open instead of smoothly animating
...
[x] Entrance currently has an open wall to nowhere. That should be a closed wall.
[x] You know that rule about having a one-cell passage between levels? That passage should be *outside* the layout of the level and not in it. For example, right now Chamber 1's one-cell passage is inside the rectangle that comprises the room. This is in error
[x] Re: Fog, you wrote: "So I’d implement top-4, remove the unused probeAmbientTexture sampler, and keep the fog pass isolated from other effects. Top-4 should cover “current level plus likely adjacent levels” without sampler-limit risk." Go do that.
[x] You wrote re: reflection cubemap streaming: "The root bug is that reflection-probe residency is still startup/level-mount centered. It needs to be player/camera centered and updated as the player moves, with a stable resident working set around the current world position." Go do that.
[x] Go ahead and implement GPU path tracing for volumetric lightmap baking.
[x] Reflection capture: Do one pass to capture cubemaps, then do another pass with those cubemaps in place applied to specular surfaces.
[x] Just like gates, whenever a player moves, any doors adjacent to their new cell must open, and all other doors must close, both in the rules engine, and in animation. Right now, when the player walks
[x]  Let's make the doors only 1.8m high instead of their current 2m high. Maintain the current texel density.
[x] Have gpt-image-2 generate PBR textures for the door instead of what we're currently using. We're looking for an ancient Minoan heavy wooden door with rusty bronze reinforcements, and a bronze ring opener. We'll want to appropriately texture each side separately, by generating textures for a 1mx1.8m left-side door with the opener on the right (since door openers are towards the middle of the door opening), then flipping it for the other side.
[x] When you walk into maze-002, you are facing forward, with a gate immediately on your right appearing closed. When you turn, the gate instantly vanishes open. The gate should have animated open the moment you walked in. The fact that it doesn't open when you walk into the maze smells suspiciously like the rules engine being level-based and not global. IT MUST BE GLOBAL. Turning should have no impact on the gate, and should have zero effect in the rules engine besides turning your character because turning does not use a turn.
[x] When I walk up to any gate, it just teleports into the ground instead of animating open. It must animate. This used to work correctly and has regressed.
[x] New gameplay element: Altar. Some level exits will have an altar next to them. The altar is a static-lightmapped 1m-cube, sitting on t he middle of the cell's floor, using the wall texture (and same texel density), with the model drop_cup_4th_century_bc.zip at 0.65m wide resting on top of it, also static-lightmapped. Convert that model to our standard ORM material.
  [x] When the player enters a cell next to the altar, if they possess a trophy, play an uninterruptible animation sequence: Bring in a black letterbox border (slide it in from top and bottom over 1s). The player turns towards the altar (1s), then places the trophy in the bowl (the bottom of the trophy resting on the bottom of the bowl) (1s), then the trophy is replaced by a blue flame (it's the torch billboard, but double the size, with its color channels swizzled, from RGB to BRG), then the letterbox goes away the same way it came in (1s), and then player control is restored
[x] Put an altar next to each maze entrance in Chamber 1, one square north of the cell in front of the entrance, so when you walk out of the maze, the altar is next to you (but not blocking the line out of the maze)
[x] When you are in maze-00N, the outer walls are unlit.
[x] Spiders are not moving correctly. The spider in maze-002, once awakened, just moves back and forth between two tiles instead of following its preferred wall all the way around the maze. I suspect it is coded to turn twice, causing it to turn 180 degrees every turn.
[x] Add credit: "Droop cup 4th century BC" (https://skfb.ly/oyB9X) by The Hunt Museum is licensed under Creative Commons Attribution (http://creativecommons.org/licenses/by/4.0/).
[x] Slide a held sword backwards 0.5m along its axis
[x] Mobile: Tapping a button should not highlight the whole third of the screen
[x] Mobile: Respond to left/right/up swipes as an alternate form of controls
[x] As a rule: Menu must fit on the screen (Primarily affects mobile but should be designed to just work anywhere)
[x] Mobile: Menu should be a hamburger menu icon, not "menu"
[x] Menu: Have tabs for different pages. The first tab is graphics settings, with controls for
  Lighting (off -> use unlit mode)
  Fog (off/on)
  Ambient Occlusion (off/on)
  The second is the level selection menu
[x] The menu must have a close button
...
[x] Monsters can not occupy the same cell. Monsters treat a cell with another monster in it as unwalkable the same way they would treat a cell across a wall from being unwalkable. Each turn's movement goes Player -> Minotaurs -> Werewolves -> Spiders (in a fixed order within each category)
[x] Saving and loading
  The game will eventually have autosave/continue functionality that lets players reload the game and get back to (roughly) the same place
...
[x] There's a rule about levels resetting when their door closes while you're not in the level. Right now, if you walk through one of the Hallway level doors, the level resets immediately, even though the door remains open both visibly (you can still see through the edge) and rules-wise (you can walk back over the edge). And if you walk back in immediately, the screen goes black (death from walking into an invisible minotaur?) and stays like that and the game is frozen. So, first, doors are like gates and should always close under the conditions that a gate would close: that there's a monster on the other side. This logic should really be the same check in practice, or at least done at the same part of the turn sequence. Secondly, I really do mean that the reset should happen when the doors close, not as a trigger on the player moving through the doors. Resetting prematurely causes things to disappear in disorienting fashion. This should not break any of the existing tests
[x] Interior levels: There should also be a 2nd floor wall above door edges like all the other edges
[x] The 2nd floor of interior levels is still getting point lights when the 1st floor wall has a torch. There should be no torches or lights on the 2nd floor.
[x] The 2nd floor of interior levels should get an ordinary 2mx2m wall piece above each door.
[x] The second floor walls and ceilings aren't occluding the moonlight during surface lightmapping. If it would help to make the ceilings two-sided that's a pretty safe fix
[x] The second floor walls are using the same lightmaps as the walls below them, which looks very wrong. They need their own lightmaps
[x] Hallway 1-5 doesn't match the diagram. It adds an extra wall next to the altar which screws up the altar animation; the player is forced to put the trophy on the altar through a wall. It's part of the introduction to the game so I'd really like to get it right. The change should not affect the solution path. Review the diagram and ask any questions if it's unclear.
[x] ShareTextures assets are using CC0, not the ShareTextures license (the ShareTextures license makes clear that we can do this because we downloaded the assets directly rather than through one of their plugins)
[x] Interior level ceiling pieces should follow the same visibility rules as interior level 2nd floor walls
